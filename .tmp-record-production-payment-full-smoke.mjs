...
const reportsDir = path.resolve('reports/production-payment-full-smoke');
...
async function seedUser() {
  const stamp = `prod-payment-full-${Date.now()}`;
...
try {
  ...
  await page.goto(`${appUrl}/payment?group=avatar`, { waitUntil: 'networkidle' });
  await page.waitForTimeout(2500);
  ...
  await context.close();
  await browser.close();
  const finalVideoPath = path.join(reportsDir, `production-payment-full-smoke-${Date.now()}.webm`);
  ...
  const result = {
    success:
      ...
      patientRow.subscription_status === 'Aktywna' &&
      /\/interview$/.test(finalUrl),
    stripeUrl,
    stripeTitle,
    ...
    seed: { email: seed.email, profileId: seed.primaryProfileId },
  };
  writeFileSync(path.join(reportsDir, 'production-payment-full-smoke.json'), JSON.stringify(result, null, 2));
