...
test.describe("Admin patient files + AI", () => {
  ...
  test("interview button disabled when no sent interview", async ({ page }) => {
    ...
    const interviewBtn = page.getByRole("button", { name: "Zobacz wyniki ankiety (wywiad medyczny)" });
    await expect(interviewBtn).toBeDisabled();
    await expect(page.getByText("Brak wysłanego wywiadu dla tego profilu.")).toBeVisible();
  });
