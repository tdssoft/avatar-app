...
test.describe("Visual review (headed)", () => {
  ...
  test("admin patient profile layout visual check", async ({ page }) => {
    ...
    await expect(page.getByText("Podsumowanie diagnozy i zalecenia dietetyczne")).toBeVisible();
    await expect(page.getByText("Pliki wynikowe dla pacjenta")).toBeVisible();
  ...
  test("user dashboard/results visual states", async ({ page }) => {
    ...
    await expect(page.getByText("Twoje zdjęcie")).toBeVisible();
    await page.waitForTimeout(PAUSE_MS);
